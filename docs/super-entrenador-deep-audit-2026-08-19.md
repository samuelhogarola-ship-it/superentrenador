...
| P0 | Entorno Vercel objetivo | Verificacion externa pendiente | Documentar variables y bloquear configuraciones Supabase incoherentes | Identificar/enlazar el proyecto Vercel correcto y conceder acceso para auditar Preview/Production | Alto: variables y despliegue remoto no comprobados | 15-30 min |
...
El 2026-08-23 se detecto que la CLI local estaba enlazada a `tiynnllrcdhsvrzsdsct` mientras la aplicacion y la documentacion apuntan a `qxugymzyvtbxeyqcvtgk`. El enlace residual se retiro y los comandos remotos ahora abortan ante cualquier discrepancia. La comprobacion del 2026-08-24 confirma que la cuenta CLI solo ve `ozexoekvshuhtkrleuze` y `tiynnllrcdhsvrzsdsct`, no el proyecto objetivo. La CLI de Vercel esta autenticada, pero este repo tampoco esta enlazado a un proyecto; por tanto, las variables y despliegues remotos requieren acceso y enlace del propietario.
