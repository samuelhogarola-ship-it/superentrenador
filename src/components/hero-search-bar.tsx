import { ChevronDown, MapPin, MonitorSmartphone, Search, Sparkles } from "lucide-react";
...
export function HeroSearchBar({ specialties, cities }: HeroSearchBarProps) {
  const paperClass =
    "grid gap-3 rounded-[28px] border border-black/10 bg-[linear-gradient(180deg,rgba(255,253,250,0.98),rgba(247,245,239,0.94))] p-3 text-[var(--ink)] shadow-[0_26px_70px_rgba(0,0,0,0.28)] lg:grid-cols-[1.05fr_0.9fr_0.78fr_auto] lg:items-stretch";
  const fieldClass =
  ...
  return (
    ...
    >
      ...
      <label className={fieldClass}>
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[var(--hot-soft)] text-[var(--accent)]">
