...
import { Building2, Home, LayoutDashboard, MapPinned, UserRound } from "lucide-react";
...
const guestNavItem = { href: "/login", label: "Acceso", icon: Building2 };
const authedNavItem = { href: "/dashboard", label: "Panel", icon: LayoutDashboard };

export function MobileBottomNav() {
  const pathname = usePathname();
  const { user, checked } = useCurrentUser();
  const navItems = [...baseNavItems, checked && user ? authedNavItem : guestNavItem];
