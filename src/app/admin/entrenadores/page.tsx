...
export default function AdminEntrenadoresPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        .select("id, slug, display_name, city_slug, headline, short_bio, specialties, photo_url, review_status, is_published, created_at, cities(name, region)")
        .order("created_at", { ascending: false });

      if (!error && data) {
        setTrainers(data as PendingTrainer[]);
      }
      setLoading(false);
    }
    load();
  ...
  async function handleReject(id: string) {
    ...
    await supabase
      .from("trainer_profiles")
      .update({ is_published: false, review_status: "rejected" })
      .eq("id", id);
    setTrainers((prev) =>
