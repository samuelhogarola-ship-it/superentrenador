...
import { coachSeed } from "../../data/coachSeed";
...
export interface CoachState extends Pick<
  AppState,
  | "users"
  | "trainer"
  | "subscription"
...
interface CoachStateContextValue {
  state: CoachState;
...
const STORAGE_KEY = "super-entrenador-coach-v1";
const CoachStateContext = createContext<CoachStateContextValue | null>(null);

function loadState(): CoachState {
  if (typeof window === "undefined") return coachSeed;

  const raw = window.localStorage.getItem(STORAGE_KEY);
  if (!raw) return coachSeed;
...
export function CoachStateProvider({ children }: PropsWithChildren) {
  ...
  const addClient: CoachStateContextValue["addClient"] = (input) => {
  ...
  const generateExport = (type: PDFExport["type"], clientId: string) => {
    setState((current) => {
      ...
      const progressEntries = current.progressEntries.filter((entry) => entry.clientId === clientId);
  ...
  const resetDemo = () => setState(coachSeed);
...
export function useCoachState() {
  ...
  if (!context) {
    throw new Error("useCoachState must be used within CoachStateProvider");
  }
